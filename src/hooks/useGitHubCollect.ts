...
export interface UseGitHubCollectOptions {
  onEvidenceReceived: (json: string) => void;
}

export function useGitHubCollect({
  onEvidenceReceived,
}: UseGitHubCollectOptions) {
  ...
  const handleFetchGitHub = useCallback(
    async (user: AuthUser | null) => {
      ...
      setCollectProgress("");
      posthog?.capture("collect_started", {
        method: user ? "session" : "token",
      });
      try {
        const body = user
        ...
        if (res.status === 202 && data.job_id) {
          ...
          throw new Error((data.error as string) || "Fetch failed");
        } else {
        ...
        posthog?.capture("collect_failed", { error: err.message });
        setCollectError(err.message || "Could not fetch from GitHub.");
      } finally {
